...
```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 1:_** *Median for førsteklasse passasjerer for avreisedestinasjon.*"}
...
```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 2:_** *Antall passasjerer per tittle.*"}
...
```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 3:_** *Kabin fordeling basert på klasse.*"}
...
### Tren modeller
Vi fant ut at de mest relevant modellene for problemet var Lasso, Random Forest og Gradient Boosting. Her implementerer vi dem, og finner de beste hyperparameterne. TODO: SKRIVE OM HVORFOR VI VALGTE PARAMETERNE. OM DET ER FORDI BEST PRACTICE, SÅ SKRIV DET.
...
print(accs)
```
Modellene har relativt lik treffsikkerhet. Ca. 85% er ikke - isolert sett - særlig imponerende. Vi tror likevel dette er omtrent hvor treffsikker en slik model kan bli gitt problemstillingen samt hvor lite datasettet er. TODO: VI VET JO DET KAN BLI BEDRE. SIER IKKE AT VI SKAL GJØRE OM, MEN ET GOOGLE SØK VISER JO DET
...
```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 4:_** *Faktorinnvirkning.*"}
vip_lso_rf <- vip_lso_rf(tuned_lso, tuned_rf, t_train)
plot_vip_lso_rf(vip_lso_rf)
```

```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 5:_** *Faktorinnvirkning.*"}
vip_xgb <- vip_xgb(tuned_xgb, t_train)
plot_vip_xgb(vip_xgb)
```

Det vi ser er at de forskjellige modellene vektlegger forskjellige faktorer. Dette ser ut til å gjelde også når vi ikke bruker set.seed(). Dette stammer trolig fra at modellene gjennomfører treningen forskjellig, spesielt mellom de tre-baserte modellene og Lasso. TODO: KANSKJE LITT MER DETALJER OM HVORFOR!!
...
```{r, echo=FALSE, fig.align='center', fig.cap="**_Figur 6:_** *Pris for klasse og avreisedestinasjon.*"}
...
### TODO: KONKLUSJON, SAMMENDRAG, NOE NOE. VET IKKE HELT.
